...
def readGFF(output, gff, allSeq):
    ...
    for line in input:
        ...
        if len(smaller) ==0:
            print ('a')
            continue
        chromosome = re.sub("NC_0*","",info[0])
        chromosome = re.sub("\..*","",chromosome)
        if chromosome == "23":
            chromosome = "X"
        elif chromosome =="24":
            chromosome = "Y"
        ...
        if info[6] == '-':
            my_seq = Seq(smaller)
            output.write(chromosome + "|" +info[4] + "|" + info[3] + "|" +str(my_seq.reverse_complement()) +"\n")
        else:
            output.write(chromosome + "|" + info[3] + "|" + info[4] + "|" + smaller + "\n")
    #output.write("\n")    
    output.close()
    input.close()    
